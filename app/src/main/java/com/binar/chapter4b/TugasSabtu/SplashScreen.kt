    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            if (requireActivity().getSharedPreferences("datalogin", Context.MODE_PRIVATE).contains("NOHP")){
                Navigation.findNavController(requireView()).navigate(R.id.action_splashScreen_to_welcome)
            }else{
                Navigation.findNavController(requireView()).navigate(R.id.action_splashScreen_to_login)
            }
            Tugas().finish()
